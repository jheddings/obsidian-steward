...
This skill ships in the `tidy` plugin. Install it to make it invocable by name (people
usually just say "dedupe"):
...
Run it from inside the target vault (the working directory is the vault root, like the
other skills here). `scripts/dedupe.py` is standalone Python 3 with no third-party
dependencies, so it can also be invoked directly.
